...
def create_todo(current_user: TokenData, db: Session, todo: models.TodoCreate) -> Todo:
    try:
        ...
        logging.error(f"Error creating todo for user: {current_user.get_uuid()}: {e}")
...
def get_todos(current_user: TokenData, db: Session) -> list[models.TodoResponse]:
    ...
    logging.info(f"Retreived all todos for user: {current_user.get_uuid()}")
    return todos
...
def complete_todo(current_user: TokenData, db:Session, todo_id: UUID) -> Todo:
    ...
    db.commit()
    db.refresh(todo)
    logging.info(f"Todo {todo_id} marked completed by user {current_user.get_uuid()}")
    return todo
